...
1b. **bot 掛掉不會有人通知你 —— 只有這個 Console 會告訴你。**
   bot 是每天 08:10 自己醒來的排程，失敗時只會安靜地往 `VoC_Bot_Log` 寫一列 `ABORT`。
   所以 Console 把「最後成功時間」和「連續失敗次數」做成常駐的健康列，連三次以上直接變紅並要你先去修資料源。
   **這代表你必須真的每天打開它**；如果你想要改成失敗時寄 email 給你，那是一行 `MailApp.sendEmail`，但會讓 bot 從「只讀 Slack、只寫試算表」變成「會對外發送」，所以我沒有自己加 —— 要的話跟我說。
...
8. **版面順序是刻意的：結論 → 三張行動卡 → 累計數字 → 資料品質 → 分頁表格 → 趨勢圖。**
   趨勢圖排最後，因為它是背景不是答案。1440×900 的第一屏會看到表格的前 3 列；13 吋筆電（1280×720）看得到第 1 列的開頭。
